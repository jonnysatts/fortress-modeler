-- =====================================================
-- MASTER DATABASE FIX - COMPREHENSIVE SOLUTION
-- =====================================================
-- 
...
DO $$
BEGIN
    RAISE NOTICE '=== FORTRESS MODELER DATABASE REPAIR STARTING ===';
...
-- =============================================================================
-- STEP 4: CREATE/FIX USER PROFILE
-- =============================================================================
DO $$
BEGIN
    RAISE NOTICE '=== STEP 4: CREATING/FIXING USER PROFILE ===';
    ...
    -- Create user profile (safe upsert)
    ...
    RAISE NOTICE '✅ SUCCESS: Created/updated user profile';
END
$$;
...
BEGIN
    ...
    PERFORM setval(pg_get_serial_sequence('projects', 'id'), COALESCE(MAX(id), 1)) FROM projects WHERE id IS NOT NULL;
...
BEGIN
    ...
    -- Check user profile
    IF EXISTS (SELECT 1 FROM profiles WHERE id = 'f7c090db-286e-41c7-aff3-7310f71fbd6d') THEN
...
-- Final confirmation
SELECT 
    'MASTER_DATABASE_FIX_COMPLETED' as status,
    NOW() as completed_at,
    'All database sync and schema issues have been addressed' as message;
