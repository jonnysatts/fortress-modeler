-- Migration: 20250718_fix_get_shared_projects_ambiguity_final.sql
-- This migration corrects the "column reference 'id' is ambiguous" error
-- in the get_shared_projects function by using a variable for the user's email.
...
BEGIN
  ...
  WHERE
      ...
      AND ps.shared_with_email = current_user_email
...
DO $$
BEGIN
    RAISE NOTICE '=== AMBIGUOUS COLUMN FIX APPLIED (FINAL) ===';
    RAISE NOTICE 'Corrected get_shared_projects() function to resolve "id" ambiguity using a variable.';
END $$;
