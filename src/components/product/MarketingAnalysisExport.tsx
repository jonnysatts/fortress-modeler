...
import useStore, { ExportDataType } from '@/store/useStore';
...
const MarketingAnalysisExport: React.FC<MarketingAnalysisExportProps> = ({ projectId }) => {
  const { registerExportFunction, unregisterExportFunction, loadModelsForProject, loadActualsForProject } = useStore();

  useEffect(() => {
    // Register the export function
    registerExportFunction('Marketing Analysis', async () => {
      // Load the data
      const models = await loadModelsForProject(projectId);
      const actuals = await loadActualsForProject(projectId);
  ...
  }, [projectId, registerExportFunction, unregisterExportFunction, loadModelsForProject, loadActualsForProject]);
